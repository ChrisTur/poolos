...
import { updatePlatformExpense, type ExpenseFormState } from "@/lib/actions/platform-expenses"
import { Loader2, Save } from "lucide-react"
import type { PlatformExpenseModel } from "@/app/generated/prisma/models/PlatformExpense"
...
export default function EditExpenseForm({ expense }: { expense: PlatformExpenseModel }) {
  const router = useRouter()
  const boundAction = updatePlatformExpense.bind(null, expense.id)
  const [state, action, pending] = useActionState<ExpenseFormState, FormData>(
    async (prev, formData) => {
      const result = await boundAction(prev, formData)
      if (!result?.error) router.push("/admin/expenses")
      return result
    },
    null,
  )
